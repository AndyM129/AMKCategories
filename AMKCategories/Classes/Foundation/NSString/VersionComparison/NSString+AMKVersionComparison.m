...
- (BOOL)amk_containsVersion:(NSString *)targetVersion {
    BOOL contains = NO;
    if (self.length) {
        // 删除两端空白符
        targetVersion = [targetVersion stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
        ...
        for (NSString *versionRange in versionRanges) {
            ...
            NSString *versionRangeBegin = versionRangeComponents.count > 0 ? [versionRangeComponents[0] stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet] : nil;
            NSString *versionRangeEnd = versionRangeComponents.count > 1 ? [versionRangeComponents[1] stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet] : nil;
            ...
            // 子版本，例如 "1.2.3" 解析为 ["1.2.3"]
                ...
                contains = [targetVersion hasPrefix:versionRangeBegin];
            }
            
            // 版本区间，例如 "1.2.3~5.6.7" 解析为 ["1.2.3", "5.6.7"]
            else {
                if ([versionRangeBegin compare:targetVersion options:NSNumericSearch] != NSOrderedDescending) {
                    if ([versionRangeEnd compare:targetVersion options:NSNumericSearch] == NSOrderedDescending) {
                        contains = YES;
                    }
                }
            }
    ...
    AMKVersionComparisonLog(@"\"%@\" %@包含指定版本 \"%@\"", self, (contains ? @"  " : @"不"), targetVersion);
    return contains;
}
...
+ (void)load {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self test_amk_containsVersion_2];
    });
}

+ (void)test_amk_containsVersion {
    AMKVersionComparisonLog(@"\n// 子版本");
...
+ (void)test_amk_containsVersion_2 {
    AMKVersionComparisonLog(@"\n\n\n// 含冗余空格的情况");
...
+ (void)test_amk_containsVersion_3 {
    AMKVersionComparisonLog(@"\n\n\n// 含冗余空格的情况");
