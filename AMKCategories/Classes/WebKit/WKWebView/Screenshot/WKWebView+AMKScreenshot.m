...
/*
 可能的方案：
 1. 滚动截屏
 2. 强制渲染 - 将 WKWebView 的 bounds 调到和内容一样大，以促使页面完全渲染后截屏
 3. 强制渲染 - 将 WKWebView 进行缩放 以至于将全部内容同时显示在屏幕上，以促使页面完全渲染后截屏
 4. 生成 PDF 后转成图片
 5. 系统API - 用 iOS11 新增的 -takeSnapshotWithConfiguration:completionHandler 方法进行长截图
 */
...
- (void)amk_takeSnapshotInRect:(CGRect)rect completionHandler:(void (^_Nullable)(UIImage * _Nullable snapshotImage, NSError * _Nullable error))completionHandler {
    ...
    if (CGRectEqualToRect(rect, CGRectZero)) {
        rect = CGRectMake(0, 0, self.scrollView.contentSize.width, self.scrollView.contentSize.height);
    }
    ...
    if (@available(iOS 11.0, *)) {
        ...
        if (@available(iOS 13.0, *)) configuration.afterScreenUpdates = YES;
